...
export default async function build_router(
    ...
    const content = fs.readdirSync(router_directory);
    // root && log("Building Router", router_directory);
    for (const item of content) {
        ...
        if (item_stat.isDirectory()) {
            ...
            sub_router.directory_full_path = path.join(router_directory, item);
            ...
            if (!!router_match) {
                ...
                if (router_name == "index") {
                    router.use(`/`, (await import(route_full_path)).default);
                        ...
                        if (!dir_router) {
                            ...
                            const additional_routes: [string, DescriptionProps][] = Object.entries(descriptions_map)
                                ...
                                .map(([entry_path, route]) => {
                                    console.log(" router alias", router_alias, "full_route", full_route);
                            ...
                            async (body, cb, event_name) => {
                                ...
                                const response = {
                                    ...
                                    json(response_body: any) {
                                        ...
                                        const compressed_response_body = zlib.deflateSync(
